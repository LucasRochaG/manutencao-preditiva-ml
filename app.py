...
# ==============================================================================
# BARRA LATERAL: MENU PRINCIPAL & NAVEGAÇÃO
# ==============================================================================
...
pagina_selecionada = st.sidebar.radio(
    "📌 Navegação Principal:",
    ["🖥️ Painel da Máquina", "🌐 Visão Geral Fábrica (Tabela)"]
)

st.sidebar.markdown("---")

# SELEÇÃO DE SETOR E MÁQUINA (EXIBIDA APENAS NO PAINEL DA MÁQUINA)
...
# FORMULÁRIO DE ABERTURA DE OS COM ATUALIZAÇÃO DINÂMICA DAS MÁQUINAS
st.sidebar.subheader("📝 Abertura Rápida de OS")

# Uso do selectbox FORA da tag form para forçar a atualização imediata da lista de máquinas
...
# Funçao de Preventiva
def get_preventiva_dados(maquina_nome):
    tag = maquina_nome.split(' ')[0]
    return {
        "tempo": "2h 00min",
        "pecas_por_tipo": {
            "🔩 Mecânica & Estrutural": [f"Conjunto de Bujões {tag}", "Anel de Guia Protetor", "Eixo Principal"],
...
# ==============================================================================
# PÁGINA 1: VISÃO GERAL DA FÁBRICA (TABELA SIMPLES E GLOBAL DE OSs)
# ==============================================================================
if pagina_selecionada == "🌐 Visão Geral Fábrica (Tabela)":
    st.title("🌐 Visão Geral da Fábrica — Tabela Global de OSs")
    st.caption("Acompanhamento centralizado de todas as máquinas da planta industrial em tempo real.")
    ...
    # 🔴 PARADA / ABERTAS
    with col_tab_aberta:
        st.subheader(f"🔴 Parada / Abertas ({len(st.session_state['lista_os'])})")
        if st.session_state['lista_os']:
            for os_item in st.session_state['lista_os']:
                ...
                st.caption(f"Prioridade: {os_item['prioridade']} | Horário: {os_item.get('hora_abertura', 'N/A')}")
                
                if st.button(f"▶️ Iniciar {os_item['id']}", key=f"tbl_iniciar_{os_item['id']}", use_container_width=True):
                    os_and = os_item.copy()
            ...
            st.success("Nenhuma OS aberta na fábrica.")

    # ⚙️ EM ANDAMENTO
    with col_tab_andamento:
        st.subheader(f"⚙️ Em Andamento ({len(st.session_state['em_andamento_os'])})")
        if st.session_state['em_andamento_os']:
            for os_item in st.session_state['em_andamento_os']:
                ...
                if st.button(f"✅ Finalizar {os_item['id']}", key=f"tbl_concluir_{os_item['id']}", use_container_width=True):
            ...
            st.info("Nenhuma manutenção em execução.")

    # ✅ RESOLVIDAS
    with col_tab_resolvido:
        st.subheader(f"✅ Resolvidas ({len(st.session_state['historico_os'])})")
        if st.session_state['historico_os']:
            for os_item in st.session_state['historico_os']:
                st.markdown(f"**{os_item['id']}** | `{os_item['setor']}` | **{os_item['maquina'].split(' ')[0]}**")
                st.write(f"✔️ {os_item['defeito']}")
                st.caption(f"Concluída: {os_item.get('hora_conclusao', 'N/A')}")
                
                if st.button(f"🔄 Reabrir {os_item['id']}", key=f"tbl_reabrir_{os_item['id']}", use_container_width=True):
                    os_reab = {
                        "id": os_item["id"],
                        "setor": os_item["setor"],
            ...
            st.info("Nenhuma OS resolvida no histórico.")

# ==============================================================================
# PÁGINA 2: PAINEL DA MÁQUINA INDIVIDUAL
# ==============================================================================
else:
