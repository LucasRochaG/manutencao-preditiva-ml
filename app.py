...
# ==============================================================================
# Base de Usuários Cadastrados (Matrícula, Nome, Perfil, Senha)
...
# ==============================================================================
# TELA DE LOGIN UNIFICADA (CASO NÃO ESTEJA LOGADO)
# ==============================================================================
if not st.session_state['autenticado']:
    ...
    with col_l2:
        st.markdown("### 🦾 MANTIS 4.0 — Acesso Industrial")
        st.caption("Insira sua matrícula e senha para entrar no sistema.")
        
        with st.form("form_login_unico"):
            mat_input = st.text_input("Matrícula (Ex: M001 ou OP101):")
        ...
        with st.expander("➕ Cadastrar Novo Funcionário"):
            with st.form("form_novo_func"):
                novo_mat = st.text_input("Nova Matrícula (ex: M003 ou OP103):")
                novo_nome = st.text_input("Nome Completo:")
                novo_perfil = st.selectbox("Perfil:", ["Operador", "Mecânico"])
                nova_senha = st.text_input("Senha:", type="password")
                btn_cadastrar = st.form_submit_button("Salvar Cadastro")
                
                if btn_cadastrar:
                    if novo_mat and novo_nome and nova_senha:
                        st.session_state['usuarios_db'][novo_mat] = {
                            "nome": novo_nome, "perfil": novo_perfil, "senha": nova_senha
                        }
                        st.success("Cadastro realizado com sucesso! Faça o login acima.")
                    else:
                        st.warning("Preencha todos os campos.")
        
        st.info("💡 **Dicas de teste padrão:**\n* Mecânico: Matrícula `M001` | Senha `123`\n* Operador: Matrícula `OP101` | Senha `123`")
...
# ==============================================================================
    ...
    st.sidebar.markdown(f"### 🦾 MANTIS 4.0")
    ...
    # ABERTURA RÁPIDA DE OS (QUALQUER USUÁRIO LOGADO PODE ABRIR)
                ...
                if os_ab:
                    for item in os_ab:
                        ...
                        if st.button(f"▶️ Iniciar {item['id']}", key=f"f_in_{setor_nome}_{item['id']}", use_container_width=True):
                            if usuario['perfil'] == "Mecânico":
                                os_and = item.copy()
                                os_and["hora_inicio"] = datetime.now().strftime('%H:%M')
                                os_and["mecanico_responsavel"] = usuario['nome']
                                ...
                                st.warning("⚠️ Apenas **Mecânicos** podem iniciar OSs!")
                        st.divider()
                else:
                    ...
                    for item in os_and:
                        ...
                        if st.button(f"✅ Finalizar {item['id']}", key=f"f_fin_{setor_nome}_{item['id']}", use_container_width=True):
                            if usuario['perfil'] == "Mecânico":
                                ...
                                st.warning("⚠️ Apenas **Mecânicos** podem fechar OSs!")
                        st.divider()
                else:
                        ...
                        if st.button(f"🔄 Reabrir {item['id']}", key=f"f_re_{setor_nome}_{item['id']}", use_container_width=True):
                            if usuario['perfil'] == "Mecânico":
                                os_reab = {
                                    ...
                                    "hora_abertura": datetime.now().strftime('%H:%M'),
                                    "autor_abertura": f"Reaberto por {usuario['nome']}"
                                }
                                ...
                                st.warning("⚠️ Apenas **Mecânicos** podem reabrir.")
                        st.divider()
                else:
                ...
                for item in os_ab_maquina:
                    ...
                    if st.button(f"▶️ Iniciar Atendimento {item['id']}", key=f"p_in_{item['id']}"):
                        if usuario['perfil'] == "Mecânico":
                            ...
                            st.warning("⚠️ Restrito: Apenas **Mecânicos** podem iniciar.")
                    st.divider()
            else:
                ...
                for item in os_and_maquina:
                    ...
                    if st.button(f"✅ Finalizar OS {item['id']}", key=f"p_fin_{item['id']}"):
                        if usuario['perfil'] == "Mecânico":
                            ...
                            st.warning("⚠️ Restrito: Apenas **Mecânicos** podem concluir.")
                    st.divider()
            else:
