...
# Módulo de Login Discreto no Canto
...
# ==============================================================================
# TELA: CADASTROS (EXCLUSIVO ADMINISTRADOR)
# ==============================================================================
if st.session_state['aba_ativa'] == "👥 Cadastros (ADM)":
    ...
    with col_cad1:
        st.markdown("##### ➕ Novo Cadastro de Usuário")
        with st.form("form_novo_user"):
            mat_c = st.text_input("Matrícula (Ex: M005, ADM02):")
            nome_c = st.text_input("Nome Completo:")
            perfil_c = st.selectbox("Perfil de Acesso:", ["Mecânico", "Administrador", "Operador"])
            senha_c = st.text_input("Senha:", type="password")
            btn_cad = st.form_submit_button("Salvar Cadastro", use_container_width=True)
            
            if btn_cad:
                if mat_c.strip() and nome_c.strip() and senha_c.strip():
                    if mat_c.strip() in st.session_state['usuarios_db']:
                        st.error("Matrícula já existente.")
                    else:
    ...
    with col_cad2:
        ...
        df_u = pd.DataFrame([{"Matrícula": k, "Nome": v["nome"], "Perfil": v["perfil"]} for k, v in st.session_state['usuarios_db'].items()])
        st.dataframe(df_u, use_container_width=True, hide_index=True)
        
        mat_del = st.selectbox("Remover Matrícula:", [""] + list(st.session_state['usuarios_db'].keys()))
        if st.button("🗑️ Deletar Usuário", use_container_width=True):
            if mat_del:
                if mat_del == st.session_state['usuario_logado'].get('matricula'):
                    st.error("Você não pode deletar seu próprio usuário logado.")
                else:
...
# ==============================================================================
    ...
    def render_tabela_setor(setor_nome):
        ...
        # OS EM ANDAMENTO (COM RELATÓRIO OBRIGATÓRIO)
        with col_and:
...
# ==============================================================================
# TELA: PAINEL DA MÁQUINA INDIVIDUAL
# ==============================================================================
else:
    ...
    with tab_andamento:
        ...
        if os_and_maquina:
            for item in os_and_maquina:
                ...
                rel_p_maq = st.text_area(f"Relatório Técnico ({item['id']}):", placeholder="O que foi feito?", key=f"p_rel_{item['id']}", height=70)
